...
#   3. the systemd units from linux/, rendered for this user, checkout, LAN
#      address and port; barza.service enabled and started
...
#
# Environment: BARZA_LAN_IP (default: the source address of the default
# route), BARZA_PORT (default 8901), BARZA_TUNNEL=1 (see above).
set -euo pipefail
...
say "systemd units (user=$USER_NAME root=$ROOT lan=$LAN_IP port=$PORT)"
render() {
    sed -e "s#@USER@#$USER_NAME#g" -e "s#@ROOT@#$ROOT#g" \
        -e "s#@LAN_IP@#$LAN_IP#g" -e "s#@PORT@#$PORT#g" "$1"
}
for unit in barza.service barza-tunnel.service barza-watchdog.service barza-watchdog.timer; do
    render "$ROOT/linux/$unit" | sudo tee "/etc/systemd/system/$unit" >/dev/null
done
